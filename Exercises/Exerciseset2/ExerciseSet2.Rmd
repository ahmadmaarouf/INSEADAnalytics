...
**Your Answers here:**
1) We need six components to get at least 50% of the variance.

2) The principal component - component weight which has weight of 21.94% has the following highest weight:
5 year T-Notes US with weight of 0.93
10 year T-Notes with weight of 0.93

3) The eigenvalue of the first 10 components is above 1, and minimum percentage of variance
is 2.04% and their cumulative is above 50%. We can call these 10 components principal components.
...
5) In the crisis, we have 13 principal components having eigenvalue greater than 1 (with weight of almost 75%).
In pre crisis, we have 15 principal components (with weight of almost 70%.
=> In crisis, few components have higher variance => explain more about the market.
...
**Your Answers here:**
1) The code takes the moving averages of futures for windows with different lengths (MAfreq).
Then, a matrix of signals is created. Weights are scaled back (normalized) to ensure
same investment per day. Decision is taken based on whether signal shifts (with 
2 days gap)


2) If you use different moving average technical indicators, for example MAfreq, the
sequence of windows taken change. Hence, the output will also change accordingly.
