...
**Your Answers here:**
1) In the dataset1.R line-8, there is mytickers = c("SPY", "AAPL"). This includes Appel returns during same period.
2) Cumulative: 596.4
round(100*sum(StockReturns[,2]),1)

Average daily return: 0.148 
round(100*mean(StockReturns[,2]),3)

Standard deviation: 2.39
round(100*sd(StockReturns[,2]),3)

3) Yes. We have to add it to line 8 of dataset.R and rerun the program to incorporate it.
...
**Your Answers here:**
1) Number of rows =2  (nrow(transposedData))
Number of columns = 4029 (ncol(transposedData))
...
**Your Answers here:**
1) We change line-9 in Dataset1.R (the start date to be changed. It will start with 2 Jan. as 1st January, there is no data)

2) Change the myticker to include some more companies c("SPY","AAPL",YHOO") etc.. and rerun:
portfolio = apply(StockReturns,1,mean)
...
**Your Answers here:**
1) It compares mydata to stockreturns for each row. If it is different, it gives 1. It gives 1 for each row, hence a total number of 20.

2) It has different dimensions and different content.
